...
public class RockerStructureViewElement implements StructureViewTreeElement, SortableTreeElement {
	protected final PsiElement element;

	public RockerStructureViewElement(PsiElement element) {
	...
	public ItemPresentation getPresentation() {
		return new RockerItemPresentation(element);
	}

	@NotNull
	@Override
	public TreeElement[] getChildren() {
		if ( element instanceof RockerPSIFileRoot) {
			Collection<? extends PsiElement> funcs = XPath.findAll(RockerLanguage.INSTANCE, element, "/script/function/ID");
			List<TreeElement> treeElements = new ArrayList<>(funcs.size());
			for (PsiElement el : funcs) {
				treeElements.add(new RockerStructureViewElement(el));
			}
